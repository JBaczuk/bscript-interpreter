module.exports = new Map([
  ...
  ['OP_1', function (context) {
    context.stack.push(Buffer.from('01', 'hex'))
    context.programCounter += 1
  }],
  ...
  ['OP_2', function (context) {
    context.stack.push(Buffer.from('02', 'hex'))
    context.programCounter += 1
  }],
  ...
  // arithmetic
  /*
  Note: Arithmetic inputs are limited to signed 32-bit integers, but may overflow their output. If any input value for any of these commands is longer than 4 bytes, the script must abort and fail. If any opcode marked as disabled is present in a script - it must also abort and fail.
  */
  ...
  ['OP_ADD', function (context) {
    ...
    const aBuf = context.popStack()
    const a = bufToInt(aBuf)
    const bBuf = context.popStack()
    const b = bufToInt(bBuf)
    const c = a + b
    const bytes = getMinBytes(c)
    const cBuf = Buffer.alloc(bytes)
    cBuf.writeUIntLE(c, 0, bytes)
    context.pushStack(cBuf)
    context.programCounter += 1
  }],
